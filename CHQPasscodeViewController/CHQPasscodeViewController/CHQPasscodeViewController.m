...
@interface CHQPasscodeViewController () <CHQPasscodeViewDelegate>
@property (nonatomic, strong) UIView      *coverView;
@property (nonatomic, strong) UIView      *animatingView;
@property (nonatomic, strong) CHQPasscodeView *passcodeView;
...
@property (nonatomic, assign) CHQLockType currentLockType;
@property (nonatomic, assign) CHQLockType aimLockType;
...
+ (CHQLockType)currentLockType
{
    return [[self sharedUser] _currentLockType];
}
...
- (CHQLockType)_currentLockType
{
    NSUserDefaults *usrDf = [NSUserDefaults standardUserDefaults];
    NSInteger type = [(NSNumber *)[usrDf objectForKey:kUserDefaultCurrentLockTypeKey] integerValue];
    return type;
}

- (void)_saveLockType:(CHQLockType)lockType
{
    NSUserDefaults *userDf = [NSUserDefaults standardUserDefaults];
    NSNumber *lockTypeNum = [NSNumber numberWithInteger:lockType];
    [userDf setObject:lockTypeNum forKey:kUserDefaultCurrentLockTypeKey];
    [userDf synchronize];
}
...
- (void)_setupFingerPrint {
    if (!self.context && _allowUnlockWithTouchID) {
        ...
        if ([self.context canEvaluatePolicy:LAPolicyDeviceOwnerAuthenticationWithBiometrics error:&error]) {
            ...
            _isUsingTouchID = YES;
            [_passcodeView anythingResignFirstResponder];
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    if (![_passcodeView anythingIsFirstResponder] && (!_isUsingTouchID || _isUserChangingPasscode || _isUserBeingAskedForNewPasscode || _isUserConfirmingPasscode || _isUserEnablingPasscode || _isUserSwitchingBetweenPasscodeModes || _isUserTurningPasscodeOff)) {
        [_passcodeView anythingBecomeFirstResponder];
//        [_passcodeTextField becomeFirstResponder];
        _animatingView.hidden = NO;
    }
...
//    [_passcodeTextField resignFirstResponder];
    ...
    {
        if(_aimLockType != _currentLockType)
        {
            _aimLockType = _currentLockType;
    ...
    [UIView animateWithDuration: _lockAnimationDuration animations: ^{
        if (_displayedAsLockScreen) {
            ...
            // Update the Keychain if adding or changing passcode
                ...
                if(_currentLockType != _aimLockType)
                    [self _saveLockType:_aimLockType];
                //finalize type switching
                if (_isUserSwitchingBetweenPasscodeModes) {
                    _isUserConfirmingPasscode = NO;
                    [self setLockType:_aimLockType inViewController:nil asModal:_displayedAsModal];
                }
            }
        }
...
- (void)updateViewConstraints {
    ...
    [_enterPasscodeLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.equalTo(_animatingView);
        make.centerY.equalTo(_animatingView).offset(yOffsetFromCenter);
    }];
    [_passcodeView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.equalTo(_animatingView);
        make.left.equalTo(_animatingView.mas_left);
        make.right.equalTo(_animatingView.mas_right);
        make.top.equalTo(_enterPasscodeLabel.mas_bottom).offset(_verticalGap);
        make.height.greaterThanOrEqualTo(@400);
    }];
    [_failedAttemptLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.equalTo(_animatingView);
        make.bottom.equalTo(_enterPasscodeLabel.mas_top);
        make.width.greaterThanOrEqualTo(@kFailedAttemptLabelWidth);
        make.height.equalTo(@(kFailedAttemptLabelHeight + 6.0f));
    }];
}
...
#pragma mark - CHQPasscodeViewDelegate
- (BOOL)validatePasscode:(NSString *)typedString {
    ...
    if ((_isUserChangingPasscode  || savedPasscode.length == 0) && !_isUserTurningPasscodeOff) {
        ...
        // Changing Passcode and the entered Passcode is correct.
            ...
            if(_isUserSwitchingBetweenPasscodeModes && _aimLockType != _currentLockType)
            {
                [self _changePasscodeViewToAimLockType:_aimLockType];
            }
...
- (void)setLockType:(CHQLockType)type inViewController:(UIViewController *)viewController asModal:(BOOL)isModal
{
    if (!_isUserSwitchingBetweenPasscodeModes &&
        ...
        _isUserSwitchingBetweenPasscodeModes = NO;
        if(_currentLockType != type)
        {
            _currentLockType = type;
        }
...
- (void)_applicationDidEnterBackground {
    if ([self _doesPasscodeExist]) {
        if ([_passcodeView anythingIsFirstResponder]) {
            _useFallbackPasscode = NO;
            [_passcodeView anythingResignFirstResponder];
        }
...
+ (instancetype)sharedUser {
    __strong static CHQPasscodeViewController *sharedObject = nil;
...
- (void)_commonInit {
    _currentLockType = [self _currentLockType];
    _aimLockType = _currentLockType;
    [self _loadDefaults];
    [self _addObservers];
}
